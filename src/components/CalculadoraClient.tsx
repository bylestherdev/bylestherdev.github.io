...
export default function CalculadoraClient() {
  ...
  // Misma matemática de siempre, solo cambia cómo se presenta
  const timeValueRecoverable = hoursPerWeek * hourlyValue * 4;
  const opportunityValueRecoverable = leadsPerMonth * ticketValue;
  const totalValueRecoverable = timeValueRecoverable + opportunityValueRecoverable;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      metricas_ingresadas: {
        ...
        valor_hora_usd: hourlyValue,
        leads_potenciales_mes: leadsPerMonth,
        ticket_promedio: ticketValue,
      },
      calculo_final: {
        valor_tiempo_recuperable_usd: timeValueRecoverable,
        valor_oportunidad_recuperable_usd: opportunityValueRecoverable,
        valor_total_recuperable_usd: totalValueRecoverable,
        ...
        <div className="w-full lg:w-1/2 flex flex-col pt-8">
          {step === 1 && (
            <div className="animate-fade-in">
              <h1 className="font-serif text-4xl md:text-5xl text-white mb-4">
                Descubre cuánto tiempo <em className="text-[#3b82f6] not-italic">puedes recuperar cada semana.</em>
              </h1>
          ...
          {step === 2 && (
            <div className="animate-fade-in">
              ...
              <div className="space-y-4 mb-8">
                {PAIN_POINTS_DATA.map((item) => (
                  <label key={item.id} className="flex items-start gap-4 cursor-pointer group">
                    <div
                      className={
                        "w-6 h-6 rounded flex items-center justify-center border mt-0.5 shrink-0 transition-colors " +
                        (painPoints[item.id as keyof typeof painPoints]
                          ? "bg-[#3b82f6] border-[#3b82f6]"
                          : "bg-[#1a2235] border-[#2d3a4f] group-hover:border-[#3b82f6]")
                      }
                    >
              ...
              <div className="flex gap-4">
                <button onClick={() => setStep(1)} className="px-6 py-3.5 rounded-lg text-[#cbd5e1] border border-[#2d3a4f] hover:bg-[#1a2235]">
                  Atrás
                </button>
                <button onClick={() => setStep(3)} className="btn-action flex-grow">
                  Continuar →
                </button>
              </div>
            </div>
          ...
          {step === 3 && (
            <div className="animate-fade-in">
              ...
              <div className="space-y-8 mb-8">
                <div>
                  <div className="flex justify-between text-[#cbd5e1] mb-3">
                    <span>Leads que podrías atender mejor si respondieras más rápido</span>
                    <span className="font-bold text-white">{leadsPerMonth} clientes/mes</span>
                  </div>
                  <input
                    type="range"
                    min="0"
                ...
                <div>
                  <div className="flex justify-between text-[#cbd5e1] mb-3">
                    ...
                    <span className="font-bold text-white text-right">
                      ${ticketValue} USD{" "}
                      <span className="text-[#94a3b8] block sm:inline font-normal text-sm">(${toCLP(ticketValue)} CLP)</span>
                    </span>
                  </div>
                  <input
                    type="range"
              ...
              <div className="flex gap-4">
                <button onClick={() => setStep(2)} className="px-6 py-3.5 rounded-lg text-[#cbd5e1] border border-[#2d3a4f] hover:bg-[#1a2235]">
                  Atrás
                </button>
                <button onClick={() => setStep(4)} className="btn-action flex-grow">
                  Ver resumen final →
                </button>
              </div>
            </div>
          ...
          {step === 4 && (
            <div className="animate-fade-in">
              ...
              <p className="text-[#94a3b8] mb-8">
                {formData.name}, a tu derecha puedes ver el tiempo y valor que podrías recuperar. Ingresa tus datos
                para enviarte este reporte detallado y habilitar la solución.
              </p>
              ...
              <form onSubmit={handleSubmit} className="space-y-5 mb-8">
                ...
                <div className="flex gap-4 pt-4">
                  <button
                    ...
                    onClick={() => setStep(3)}
                    className="px-6 py-3.5 rounded-lg text-[#cbd5e1] border border-[#2d3a4f] hover:bg-[#1a2235] disabled:opacity-50"
                  >
                    Atrás
                  </button>
                  <button
                    type="submit"
                    disabled={isSubmitting}
                    className="btn-action flex-grow !bg-[#10B981] hover:!bg-[#059669] disabled:opacity-50 disabled:cursor-not-allowed"
                  >
            ...
            <div className="animate-fade-in lg:text-left pt-6">
              <p className="text-[#cbd5e1] mb-8 text-xl leading-relaxed">
                ...
                <strong className="text-[#10B981]">
                  ${totalValueRecoverable.toLocaleString()} USD (${toCLP(totalValueRecoverable)} CLP)
                </strong>{" "}
        ...
        <div className="w-full lg:w-1/2 sticky top-32">
          <div className="bg-[#111827] border border-[#2d3a4f] rounded-2xl p-8 lg:p-10 shadow-2xl">
            <span className="text-[#94a3b8] text-xs font-bold tracking-widest uppercase mb-8 block">
              Calculadora de Tiempo Recuperable
            </span>

            <div className="space-y-8 mb-10">
              <div>
                <div className="flex justify-between text-[#cbd5e1] mb-3 text-sm">
                  <span>Horas en tareas manuales por semana</span>
              ...
              <div>
                <div className="flex justify-between items-end text-[#cbd5e1] mb-3 text-sm">
                  <span>Valor estimado de una hora de tu tiempo</span>
                  <div className="text-right">
                    <span className="font-bold text-white">${hourlyValue} USD</span>
                    <span className="block text-[#94a3b8] text-xs">(${toCLP(hourlyValue)} CLP)</span>
                  </div>
                </div>
                <input
                  type="range"
            ...
            <div className="bg-[#1a2235] border border-[#2d3a4f] rounded-xl p-6 relative overflow-hidden transition-all duration-300">
              ...
              <p className="text-[#cbd5e1] text-sm mb-2">Podrías recuperar aproximadamente</p>
              ...
              <div className="flex flex-col mb-6 border-b border-[#2d3a4f] pb-4">
                <div className="flex items-baseline gap-2">
                  <h3 className="text-4xl md:text-5xl font-bold text-[#10B981] transition-all">
                    ${totalValueRecoverable.toLocaleString()} USD
                  </h3>
                  <span className="text-[#94a3b8]">/mes</span>
                </div>
                <span className="text-[#10B981] font-semibold text-lg">(${toCLP(totalValueRecoverable)} CLP)</span>
              </div>

              <div className="space-y-4 text-sm">
                <div className="flex justify-between items-center">
                  <span className="text-[#94a3b8]">Valor del tiempo en tareas manuales:</span>
                  <div className="text-right">
                    <span className="text-white font-medium block">${timeValueRecoverable.toLocaleString()} USD</span>
                    <span className="text-[#94a3b8] text-xs">(${toCLP(timeValueRecoverable)} CLP)</span>
                  </div>
                </div>
                {opportunityValueRecoverable > 0 && (
                  <div className="flex justify-between items-center border-t border-[#2d3a4f] pt-4">
                    ...
                    <div className="text-right">
                      <span className="text-[#10B981] font-medium block">${opportunityValueRecoverable.toLocaleString()} USD</span>
                      <span className="text-[#10B981]/70 text-xs">(${toCLP(opportunityValueRecoverable)} CLP)</span>
                    </div>
                  </div>
                )}
              </div>
